...
**One over-exposure is recorded and not yet fixed.** `CompiledRuleSet.network()` puts the whole compiled node graph on a contract interface, written fully qualified — the one such reference in `-core`. Nothing outside this repository needs it; §7.4's `CompilerReport` is the supported introspection, and the only callers are `-core` itself and the testkit's white-box structural tests. Removing it from the interface is pre-publish work this pass did not do. It is named in the test's allowed list as well as here, because prose does not fail a build and a second one would arrive as quietly as the first.
