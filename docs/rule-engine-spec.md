...
| 6 (stretch) | Negation/`NOT_EXISTS` ✔, `FOR_ALL` ✔, truth maintenance ✔, accumulate ✔, temporal/CEP, distributed sharding | `NOT_EXISTS` is built (§1's amendment), so is `FOR_ALL` and `ACCUMULATE` (§2.5's two), and §4.4's amendment pays off the boundary the quantifiers shipped with. `collect`, temporal/CEP and distributed evaluation are out of scope; §1 says what each costs and what the interim answer is |
