...
- **Accumulation / aggregation (`ACCUMULATE`).** `sum`, `count`, `collect`, `average` over matching facts. Same reasoning — incremental aggregate maintenance under retract is its own correctness problem. **Interim answer:** aggregate at ingestion, insert the aggregate as a fact. **Built, except `collect` — see §2.5's second amendment.** The correctness problem this bullet names never arose, for the reason negation's machinery never did: nothing is maintained incrementally, so there is nothing to maintain under retract.

- **Backward chaining.** Forward-chaining only.

- **Full truth maintenance** (logical retract cascades, justification graphs). Land plain assert/retract first. Note the ordering dependency: truth maintenance and negation want to land together, since the strongest motivation for the former is retracting matches justified by an absence. **Built — see §4.4's amendment.** It landed two slices *after* negation rather than with it, which cost both quantifiers a documented boundary in the interim.

- **Temporal/CEP operators** (sliding windows, `after`/`within` sequencing). Leave room in `JoinConstraint`/`Operator`; don't build now. **The sequencing half is built — see §2.5's third amendment.** The room left in `JoinConstraint` and `Operator` is exactly where it went. Sliding windows are not, and cannot be without a clock: this engine reads no time it was not given. Time-windowed *eviction* is a separate concern from temporal *operators*, and §4.4's note on session growth surfaces argues the eviction half is needed sooner than this bullet implies — it is what bounds a long-lived session's memory, with or without temporal operators.
...
> **Amendment (Phase 5).** What shipped is `com.codeheadsystems.rules.schema.FactSchemas` (`-core`), not the `SchemaRegistry` sketched above, and the two do not share a shape. `FactSchemas` never hands back a `JsonSchema` at all: `violations(factType, payload)` answers the insert-time question above, and `typeOf(factType, dottedPath)`/`presence(factType, dottedPath)` answer the compile-time question in this engine's own vocabulary — a `SchemaType` or `Optional.empty()`, a `Presence` or `Presence.UNKNOWN` — rather than exposing networknt's `JsonSchema` on a `-core` interface. That is the point: `JsonSchema` on a public `-core` signature would put `com.networknt:json-schema-validator` on every consumer's classpath, including one that registers no schema at all. `typeOf`/`presence` answer "don't know" (empty / `UNKNOWN`) wherever schema introspection stops being simple — `$ref`, `allOf`, `oneOf` — which costs exactly what an unregistered schema already costs: the compiler checks less. `FactSchemas.none()` is the zero-setup default a `CompiledRuleSet` carries otherwise. The implementation backed by networknt's validator, `JsonSchemaFactSchemas`, lives in the optional `rule-engine-schema` module and is wired in through `CompilerOptions`, the same SPI pattern `TestedPaths`, `HostFunction` and `EventSink` use — `-core` gains no dependency for it.
...
> **Second amendment (Phase 6, fourth slice, as built).** **`ACCUMULATE` is implemented**, for `sum`, `count`, `min`, `max` and `average`. `collect` is not, backward chaining and the temporal operators are not, and §1's bullets stand for them unchanged.
>
> **The hard problem this was deferred for never arose.** §1 defers accumulation because "incremental aggregate maintenance under retract is its own correctness problem" — and it is, for an engine that maintains aggregates incrementally. This one does not. The fold is computed from working memory each time it is read, so a retract has nothing to correct: the next read simply sees one fewer fact. That is the third time this phase that a feature priced by §1 cost a fraction of the estimate, and the reason is the same each time — a question asked of a *complete* tuple needs none of the machinery a question answered incrementally does.
>
> **The alias binds a value, and the value is never stored.** This is the one place §3.2.2's invariant — "tuples bind `FactHandle`s, never `Fact` objects" — came under real pressure, because an aggregate is the most stale-able value in the engine: any fact in its scope moving makes it wrong. Binding it into the tuple would have broken the invariant outright under §4.3's shape, where a tuple is materialised and held across cycles. So an accumulate alias names a value that `Accumulators` folds at each read, exactly as `Tuple.payloadOf` dereferences a handle at each read. The cost is that a rule reading the same accumulate twice folds it twice, which is the trade `Negations` and `Universals` already make.
>
> **Every constraint selects the scope**, joins and literals alike — where a `FOR_ALL` splits them because it has a requirement to state about each fact. An accumulate has no such half, so "the total of the *physical* line items of this order" is expressible here and, as §2.5's first amendment records, is not expressible with a universal. The `having` is what tests the answer.
>
> **It takes no tuple position, which decides what may read it.** A `having`, a `$ref` in an action and a §6.4 expression may; a join from another pattern may not, because a join compares two facts and there is no fact on this side. The compiler says exactly that rather than reporting the alias as unbound — an author who reaches for it in a join is right to want the value and wrong about where.
>
> **Two arithmetic decisions that are semantics, not implementation.** A fact whose field is absent or non-numeric is **skipped, not counted as zero**, because §2.6.1 is emphatic that absent is not zero and the arithmetic has to agree with the matching semantics or the same rule set means two things — so the average over a scope where half the facts lack the field is the average of the half that have it. And an **empty scope** answers zero for `count` and `sum`, which are their identities, but *absent* for `min`, `max` and `average`: the mean of nothing is not a number, and answering zero would make "average below 10" true for a customer with no orders, which is `FOR_ALL`'s vacuous-truth trap in arithmetic form.
>
> `sum` and `average` fold through `BigDecimal` and the scope is walked in working-memory order (insertion order per type), so §7.3's determinism is structural rather than argued. The one place the walk order is visible at all is `min`/`max` over a scope mixing strings and numbers, which §2.6.1 does not order against each other; the running extreme is kept when they cannot be compared.
>
> **`collect` is the one left out, and deliberately.** It answers with a collection rather than a scalar, so it has no meaningful `having`, and a rule that could bind a list would need a way to take one apart that §2.5 does not have. §1's flattening advice is the interim answer; adding it later is a new constant plus a value-semantics decision, not a reshape.
>
> **Never accumulate over a type the session evicts** (§4.4). This is the fourth member of the family and arguably the worst: evicting a negated type manufactures a false conclusion and evicting a quantified one deletes a requirement, but evicting an accumulated type quietly changes a *number* — and a total that is wrong by the amount that was evicted is harder to notice than a rule that stops firing.

> **Third amendment (Phase 6, fifth slice, as built).** **Bounded temporal sequencing is implemented**, as `AFTER` and `BEFORE` with a required `within`. Sliding windows, `collect`, backward chaining and distributed evaluation are not.
>
> **Time is a field, and this engine owns no clock.** That is the decision every other property follows from, and it is a §7.3 decision before it is a feature decision: the determinism contract says the same rule set over the same facts in the same order fires the same way *on every host and run*, and a wall clock ends that — a slower machine sees different intervals, and a test that passed in January fails in July. Reading time from the facts means the guarantee survives untouched, and a session replayed in 2170 fires exactly as it did in 1970.
>
> **The bound is in the time field's own units**, because only the author knows what they are. A field holding epoch millis takes a window in millis; one holding epoch seconds takes seconds; one holding a sequence number takes a count. Assuming any of those would be assuming wrong for a large share of rule sets, silently, and a silently wrong interval is worse than an explicit one.
>
> **The bound is required, and that is the whole of what the operator adds.** An unbounded "after" is `gt` against the same `$ref` and this language already has it, so an `AFTER` that allowed the window to be omitted would be a second spelling for an existing operator. What no pair of comparisons can state is `other < mine <= other + window`: it needs the other fact's value *twice*, and a constraint reads it once. Same argument `FOR_ALL` and `ACCUMULATE` each made for their own existence.
>
> **The bound must be positive, not merely non-negative.** Zero is empty by construction — the near edge is strict, so `after within 0` is `other < mine <= other` — and unlike a range there is no reading under which an author meant it. Refused by the schema and again by the compiler, which is the shape §6.2.3's three gates take for everything the schema can state.
>
> **Strict on the near side, inclusive on the far one.** Two facts sharing a timestamp are not "after" each other — a sequence is what an author means, and an inclusive near edge would make each fact after the other if the rule were written both ways — while "within 24 hours" includes the twenty-fourth hour exactly, because that is what the phrase means out loud.
>
> **It rides §6.2.3's reserved extension.** That section reserves `{ $ref: …, extension: … }` and names `transform` as the hypothetical; `within` is the first one actually implemented, and it belongs there rather than as a sibling constraint because the bound is part of the same *relation* rather than a second condition on the same field. Every other extra key is still refused, so the reservation stays a reservation.
>
> **Temporal joins are never index-eligible**, and it is worth being exact about which gate makes that true, because the obvious answer is the wrong one. `JoinEnumerator.probe` declines `AFTER` and `BEFORE` in its default branch and `NetworkBuilder` never records an index for them: those are the load-bearing gates. `Operator.reversed()` also declines — a reversal returns an operator alone and would leave the bound behind — but that is a second belt rather than the first, established by mutation: returning the converse there leaves the suite green, because nothing can reach a probe with it. The reason to decline at all is *cost* rather than correctness. A window-dropping probe is the too-wide case, and every join is re-applied afterwards, so it would be slow rather than wrong; §3.3's invariant runs the other way, where a probe returning too *few* candidates is the lost firing. An unbounded range probe simply narrows nothing worth the walk. §7.4's report names it like any other unindexable constraint.
>
> **The window is normalised to a decimal**, and `DslEquivalence` is why. A duration of 86400000 reaches `JoinConstraint` as an `IntNode` from a rule file and a `LongNode` from the Java builder, and Jackson's node equality is type-sensitive — so the same rule written two ways produced two version hashes, which §5.6's hot reload, refraction and `RuleSetFingerprint` all key on. A duration is a magnitude rather than a typed value, so collapsing the width is lossless. Third defect this module's oracle has surfaced, and the same shape as the first: `RangeConstraint`'s un-normalised inclusivity.
>
> **What is not built, and cannot be on these terms.** A sliding window ("only the last 24 hours of `Payment`") and an absence over time ("nothing has happened for 24 hours") both need something to notice that time has *passed with no fact arriving*. Nothing here can: the engine acts when facts move, and no fact moving is exactly the input it never receives. Both need either a clock or a caller-driven session time, and both are a §7.3 decision rather than an implementation detail — a clock read inside the engine would make the firing sequence depend on when it ran. §1's interim answer stands: stamp the facts at ingestion, and let whatever already drives your stream decide what is stale.
...
> **Amendment (Phase 6, third slice, as built).** **Truth maintenance is implemented**, for logically-inserted facts. Backward chaining and distributed evaluation are not, and §1's bullets stand for them unchanged. *(`ACCUMULATE` and the temporal operators were not implemented when this was written; §2.5's second and third amendments landed them, and the first of those overturns a paragraph below — flagged where it stands.)*
>
> **A conclusion is declared, not inferred.** `insertFact` gains a `logical` flag (`insertLogical` from the builder, `logical: true` in a rule file); absent means false, which is what every insert written before this meant and must go on meaning. Only a flagged insert is withdrawn. Making every derived fact revocable was the alternative and would have been a silent behaviour change to every existing rule set — a fact a rule inserted deliberately, to be retracted by another rule later, would start vanishing on its own.
>
> **Validity is re-asked of the tuple, not diffed against a match set.** The obvious implementation compares justifications against the rule's current matches, and it does not work: §4.3's streaming conflict set holds only what is *waiting to fire*, so a match that fired reads as gone and every conclusion would be withdrawn the instant it was drawn. `TupleMatch.holds` re-asks the question of the bound handles directly — every handle present, every alpha test, every join, §1's implicit inequality, then `NOT_EXISTS`, `FOR_ALL` and §6.4's conditions in the order `RecomputingAgenda` applies them. It shares all four predicates with the agenda rather than restating them, which is why those classes are public: a revalidation that disagreed with the matcher about the same tuple would either leave a conclusion standing after its reason died or retract one the engine still believes.
>
> **Refraction is cleared when a justification dies, and the coupling is load-bearing.** A justification dies because its tuple stopped matching; if that tuple starts matching again — the `Payment` blocking a negation is itself retracted — the handles are unchanged, so the `ActivationKey` is unchanged, and a rule still refracted on it would never re-fire and never re-conclude. A conclusion withdrawn by something temporary would stay withdrawn for the life of the session.
>
> **It runs at quiescence, at the top of a fire cycle, where §4.4 already runs eviction** — and after it, because an eviction can itself be what kills a justification. Never between §4.6's staging and commit, for the reason eviction is barred from there. So a conclusion outlives its reason until the next cycle boundary, which is the honest description of when this engine withdraws things.
>
> **One forward pass cascades completely, and the argument is worth keeping.** Justifications are visited in the order their rules fired. A justification binds only facts that existed when it fired, so its positive dependencies were concluded *earlier* in that order. Withdrawal only removes facts, and removing a fact can never invalidate an earlier justification it does not bind — it can only make a `NOT_EXISTS` more satisfied, a `FOR_ALL` more satisfied, and a condition reads bound payloads alone. A `while (changed)` loop was written first and removed once no mutation of it could be made to fail a test. What would break the argument: a conclusion shared by two justifications, or a withdrawal that inserted rather than only retracting.
>
> ***Superseded by §2.5's second amendment: the loop is back.*** `ACCUMULATE` falsifies both halves of the paragraph above. Removing a fact *decreases* a `count` or a `sum`, so a withdrawal can make an **earlier** justification's `having` stop holding rather than start; and a fold reads its scope from *current* working memory rather than from the handles the tuple binds, so a justification can depend on a fact that did not exist when it fired. `TruthMaintenance` repeats until a round changes nothing, and `touchedTypes` is deliberately not cleared between rounds. The argument above is kept because it is exactly what a future reader would otherwise re-derive and re-apply.
>
> **Never evict a type your rules conclude**, which is the third member of the family §1 and §2.5 already name. Evicting a conclusion forgets its justification while that justification still holds, and the rule that drew it is still refracted — so it never comes back. Where evicting a negated type manufactures a false conclusion and evicting a quantified type deletes a requirement, evicting a *concluded* type silently loses one and leaves nothing able to redraw it. Cap the types you assert, not the ones your rules derive.
>
> **A conclusion that defeats its own reason oscillates, and the cycle limit is the defence.** `notExists Alert(...) → insertLogical Alert` concludes, the conclusion defeats the negation, the conclusion is withdrawn, refraction is forgotten, and it concludes again. That is what truth maintenance *means* for such a rule rather than a defect in it, so nothing tries to detect it — but `notExists` plus `insertFact` is the "do this once" idiom the guide teaches, and adding `logical: true` to it turns a terminating rule set into a §4.7 cycle-limit breach. Worth knowing before reaching for the flag.
>
> **A re-firing supersedes its own conclusion.** An update that clears refraction while leaving the match valid fires the same activation again, and what it concluded before was drawn from the old payload. The earlier conclusion is retracted at the next quiescence point rather than left standing beside the new one — not at the firing that replaced it, because §4.6 bars a retraction mid-commit.
>
> **Where this departs from classical truth maintenance: there is exactly one justification per conclusion.** A logical insert allocates a fresh handle (§6.2.2), so two matches concluding "the same" thing produce two *facts*, each withdrawn with its own reason — not one fact held up twice. A multi-support graph was written first and removed as unreachable. The consequence is that two unpaid orders for one customer yield two `CustomerAtRisk` facts and a rule counting them counts two. Deduplicating conclusions by content is a **separate feature** from withdrawing them: it needs an equality index over payloads and it changes what §2.1 means by fact identity. Aggregate or flatten at ingestion, which is §1's answer to the same shape of question.
...
- **`callFunction` handlers are untrusted for *time*, not just for effects.** A handler that blocks indefinitely stalls the session for as long as it blocks, and there is no fire-loop timeout to rescue it (§4.7) — not in production, and not in strict mode either; §4.7's wall-clock bound is still the open decision recorded there. Document the contract (non-blocking, bounded) at the host boundary until a configurable per-call timeout is built.
...
> **Amendment.** As of `dev.cel` 0.14.0, it ships neither. `CelExpressions` (`-cel`) substitutes its own structural estimate at compile time — an AST-node-count budget checked via `Program.estimatedCost()` — and, at runtime, `dev.cel`'s `comprehensionMaxIterations` and parse limits as the backstop, and says so in its own Javadoc. The compile-time reject is opt-in, not automatic: `CompilerOptions.expressionBudget` defaults to `Long.MAX_VALUE`, so nothing is rejected until a host sets a budget — "estimate at compile time" ships by default, "reject expressions over a configured budget" only once one is configured. Determinism is a property of that environment, not an extra guarantee: CEL's standard function set has no clock, and `CelExpressions` binds only the tuple's aliases into it, so adding any other binding there is a §7.3 decision.
...
| A host-owned list or feature store consulted *live* during matching, from a `HostFunction`, a CEL binding or a custom `ExpressionCompiler` | Not live. A snapshot bound for the session's lifetime is an input like any other and threatens nothing here; a structure that changes under a running session is the threat. Look it up before the session and insert the answer as a fact; a rule that changes the list writes the fact and emits the change (§1's lookups bullet). Then the session's inputs *are* the facts, and a replay that supplies the same facts reproduces the decision whatever the store says today |
...
| The compiled rule set's literal fingerprint is re-hashed at session creation and compared against the one taken at compile time | Invariant 1 — a literal (`FieldConstraint`/`RangeConstraint`/`Literal`) reached and mutated through `CompiledRule.source()`, which every session shares with zero synchronization | §5.5 |
| `EvictionPolicy.selectVictims` is called twice on the same working memory and the two selections must be equal | A policy reading something outside the view it was shown — a clock, a counter, a `HashMap`'s iteration order — which §7.3 forbids reaching the firing sequence | §4.4, §7.3 |
| Every handle an eviction policy selects is checked against working memory before anything is retracted | A policy inventing a handle, as opposed to a listener racing an eviction already in progress | §4.4 |
...
### 8.1 The module boundary, and why it is a test rather than a `module-info`

A module layout says which code lives where. It does not say which of that code is a **promise**, and Java has no `internal` to say it with — a type one package needs from another has to be `public`, and a consumer cannot tell that apart from the API. §2.5's three quantifiers alone made six types and three members public purely so a sibling package could reach them: `Negations`, `Universals`, `Accumulators`, `Conditions`, `TupleMatch` and `Scan`, plus `Canonical.orderable`, `RefractionMemory.forget` and `Agenda.reactivate`. Every one was right locally and none is API. Before a first publish that costs nothing; after one, each is a compatibility surface for the life of the artifact.

**JPMS is the right mechanism and is deferred, for a narrower reason than it first appeared.** A `module-info` with qualified `exports … to` clauses would let javac enforce this in a consumer's build as well as ours. Writing one for `-core` fails with `package com.google.re2j is not visible`, and the obvious reading of that — "re2j has no module name, so it cannot be required" — **is wrong**, which is worth recording because it is the reading a reader arrives at unaided. The JDK derives an automatic module name from the jar filename: `re2j@1.8 automatic` appears in `--list-modules`, and `requires re2j` compiles against the module path. What actually happened is that *Gradle* only promotes a jar to the module path when it carries a descriptor or an `Automatic-Module-Name`, so re2j stayed on the classpath — a build-configuration problem, fixable from this side with `extra-java-module-info` or by forcing the module path.

The real objection is the one that survives fixing that: **a published library must not `requires` a filename-derived automatic module.** The name is not re2j's to promise, it changes if the artifact is repackaged, `jlink` refuses such a graph, and every consumer inherits the constraint. Adding a module descriptor is a promise about the whole dependency graph, and one link of ours will not make it. The optional `-cel` module has a harder version of the same problem: dev.cel **splits packages across jars** — `dev.cel.common`, `dev.cel.common.ast`, `dev.cel.common.types` and others each appear in more than one artifact — which is a hard JPMS error that no naming convention fixes.

**The boundary is therefore enforced by a test**, `ApiSurfaceTest`, in the idiom this project already uses wherever a compiler cannot check something. It names which `-core` packages are the contract, and which internal ones each sibling module may reach into — the `exports … to` table, written where a reviewer sees it. It is weaker than javac in one way, binding this repository rather than a consumer's, and stronger in another: widening the surface becomes an edit to a named list rather than the invisible side effect of typing `public`. It also asserts the reverse — a grant nothing uses is removed, so the table keeps meaning what it says — and pins re2j's packaging — by reading the jar for a descriptor and an `Automatic-Module-Name`, not by asking the runtime, which reports only where a jar was *placed* and answers "unnamed" for everything on the classpath including jackson. The day re2j declares a name of its own, that test fails and the answer is to delete it and write the `module-info` instead.

**The pass moved two packages, and drew the boundary wrongly twice on the way.** The six types above lived in `match`, beside `Activation` and `ActivationKey` — which *are* contract, named by §7.1's listener interface, `FireRecord` and `RhsErrorHandler`. One package holding both meant the boundary could not be drawn at package granularity, which is the granularity JPMS gives you and the one the test enforces, so the predicates moved to `eval`, taking a seventh, already-package-private helper with them.

The first draft of the table then made the opposite error, listing `access`, `agenda` and `match` as internal while an exported signature named each of them: a consumer cannot implement a listener, supply a conflict-resolution strategy or walk a compiled rule without them. A table that says a package is internal while the API names it is worse than no table. The second draft over-corrected on `agenda`, exporting the whole package to reach the one plug point it held — which publishes `Agenda`'s twelve members and `RefractionMemory`'s mutating session state, `Agenda.reactivate` and `RefractionMemory.forget` among them, two of the nine things this section opens by naming as *not* API. The fix is the same split `eval` got, in the other direction: `ConflictResolutionStrategy` and its default are a `Comparator<Activation>` and belong beside the `Activation` they compare, so they moved to `match` and `agenda` is internal.

**What made both errors invisible was that nothing read `-core`'s own signatures.** The table was consistent with itself either way; only a check of whether an exported signature names an internal type could say it was false, and the first draft had no such check. It has one now, over declared types by reflection rather than over text — `Comparator<Activation>` and `Optional<Tuple>` are found because it reads the generic type name, which is the form a consumer's compiler sees.

**One over-exposure was recorded here and is now fixed, at 1.0.0.** `CompiledRuleSet.network()` put the whole compiled node graph on a contract interface, written fully qualified — the one such reference in `-core`. Nothing outside this repository needed it; §7.4's `CompilerReport` is the supported introspection, and the only callers were `-core` itself and the testkit's white-box structural tests. This section called removing it pre-publish work, and a first publish is what made it urgent rather than tidy: after one, each of the three signatures naming `Network` is a compatibility surface for the life of the artifact, and a method cannot be taken off a published interface inside a major version.

The fix is the same move `eval` and `match` got, applied to the other half of `session`. That package held both the contract — `CompiledRuleSet`, `RuleSession`, `SessionOptions`, `FireResult` — and the two classes implementing it, and the boundary is drawn at package granularity, so no amount of care could say "the interface is API and its implementation is not". `DefaultCompiledRuleSet` and `DefaultRuleSession` moved to a new internal package, `runtime`, granted to `-compiler` because the compiler is what constructs a rule set; `RuleSetFingerprint` and `SessionEvictor` came with them, both already package-private and reached only by those two. `network()` is then an ordinary public method on an internal class, which is what it always was in substance, and the interface no longer declares it at all.

Two more went with the move and neither was found by looking for `Network`. `SessionIds` — fifteen lines generating the one UUIDv7 a session carries — was `public` in `session` for exactly the reason this section opens with, so that a sibling could call it, and its own documentation schedules its deletion for whenever the build reaches JDK 26 and `UUID.ofEpochMillis` exists. Published, that deletion becomes a binary-incompatible removal: a stopgap would have dictated a major version. It is package-private in `runtime` now, and it is the only type in the whole move whose visibility actually shrank. `DefaultRuleSession` is package-private too; only `DefaultCompiledRuleSet` has to be public, because `-compiler` constructs it. The general lesson is that "what does an exported signature name" and "what is public only so a sibling can reach it" are different questions, and the reflective check answers the first — the second still needs somebody to look.

What this cost, and it is worth naming because it is the argument for doing it before a publish rather than after: nothing. Every caller was inside this repository, `RuleCompiler` changed one import, and the one white-box test that walks the graph casts to the implementation — a thing a test may do and main source may not. `ApiSurfaceTest`'s allowed-leak list is now empty and the two assertions over it are kept, because the mechanism that stops a second allowance arriving quietly should outlive the particular debt it was built for.

**One thing that pass turned up in passing.** The suites that read files rather than classes — `DocExamplesTest` over the docs, `ApiSurfaceTest` over the sibling source trees — had none of those files declared as task inputs, so Gradle marked the task `UP-TO-DATE` and the guard ran against the previous build's answer. CI never saw it, because CI always starts clean. Found by mutating a sibling module and watching the guard not run. The three doc fixtures are declared in the convention plugin, since `-cel`'s own doc suite had the same hole and naming only the module that needed it today would let the next one reopen it; the seven source trees, and the `settings.gradle.kts` that test reads the module list from, are declared in the testkit's build file instead, because as a global input a comment edit in `-cel` would re-run `test` and `strictTest` in six other modules for a guard that lives in one.
...
| 3 | Streaming sessions: `SessionOptions`-selectable persistent beta memory (Rete `JoinNode`), the Rete agenda shape and its `activate`/`deactivate` interface (§4.3), `fireUntilHalt` + hardened `SessionActor` (§5.4), session fact-eviction (§4.4) — **not** differential propagation (§11.2): §11.2's condition for reversing (A′) was measured in this phase and found met, but whether to build it stayed open — recorded as measured and not built, not as a rejected measurement — see §11.2's amendment | Long-lived session with streaming inserts amortizes join cost; **TREAT and Rete produce identical firing sequences on the same input** (§11.5) — established by differential test against the v1 engine, which by then is a shipped oracle, not a thought experiment; a streaming session under sustained insert-without-retract load reaches a steady-state heap |
...
| 6 (stretch) | `NOT_EXISTS` ✔, `FOR_ALL` ✔, truth maintenance ✔, `ACCUMULATE` ✔, temporal sequencing ✔ — `collect`, sliding windows, backward chaining and distributed evaluation **not built** | **All three matchers produce identical firing sequences for every quantifier**, established by differential test rather than by construction alone — each behavioural case asserts *what fired*, not only that they concur. And every slice is mutation-tested: **delete the feature and the suite must fail.** A test that passes against a deleted feature is the failure mode this phase kept finding, and the criterion that catches it. §9.1 records what landed, what did not, and on what terms |

### 9.1 Phase 6 as it actually landed

Five of Phase 6's items are built and four are not, and the gap between what §1 predicted each would cost and what it cost is the most useful thing this phase produced.

**What §1 priced, and where the rest of it landed.** Negation was deferred because "a `NotNode` needs per-tuple match *counters*, correct behavior when the count crosses 1↔0 in both directions, and correct interaction with truth maintenance". Two of those three were unneeded; the third arrived exactly as predicted, two slices later, and cost a justification graph. Accumulation was deferred because "incremental aggregate maintenance under retract is its own correctness problem" — and no aggregate is maintained here, so no memory needed correcting; but the problem itself did surface, in the truth-maintenance pass, where a retract that moves a total invalidates a justification the pass has already walked past. Both estimates were right about the feature they described and priced a fraction of the feature that got built, for the same reason each time: **a question asked of a *complete* tuple needs none of the machinery a question answered incrementally does.** `NOT_EXISTS`, `FOR_ALL` and a `having` are predicates over a finished match; an accumulate is a fold computed at read time. Those four are applied in `RecomputingAgenda`, the shared base — and the predicates themselves live in `Negations`, `Universals`, `Accumulators` and `Conditions`, public precisely so §7.2's explainer and `TupleMatch`'s revalidation ask them rather than restate them. That is what makes the three matchers, the explainer and truth maintenance agree about a tuple by construction rather than by testing.

That is the reusable lesson, and **this phase supplied its own counterexample rather than leaving it hypothetical.** The shape works because those four maintain no state. Truth maintenance does: a justification graph, a touched-type set carried across rounds, a supersede queue — and it needed the machinery §1 priced, plus a refraction coupling, plus a cascade loop that had to be restored once `ACCUMULATE` broke the argument for removing it. It is also the one slice where the three matchers genuinely *did* diverge, and had to be reconciled with a new agenda seam (`Agenda.reactivate`) and a Rete-specific override. Anything that holds state is priced at §1's estimate, not at the other four's; a sliding window would hold state.

**The four boundaries, which are one boundary.** Every slice collided with §4.4's eviction, in a different way each time: evicting a negated type manufactures a false conclusion, evicting a quantified type deletes a requirement, evicting an accumulated type quietly changes a *number*, and evicting a concluded type loses a belief nothing can redraw. Each amendment calls its own the sharpest, which is a sign that ranking them is not useful — they are four faces of one fact, that the engine cannot tell an evicted value from one that was never there. It re-asks the same question of the same working memory and is fooled identically, so it cannot **detect** any of the four — but §7.2's explainer **warns** for all four, naming the count and what that particular hazard costs, because what an eviction does to a rule differs by category. A negated or quantified type manufactures a match; a concluded one silently loses a belief that will not be redrawn, since its justification still holds and its rule is still refracted; a folded one changes a *number*, which is the only one of the four that can cost a firing as readily as it can hand one over — so it is also the only one that appears on the verdicts where the rule found nothing. **Never cap a type your rules negate, quantify over, fold, or conclude.**

**What is not built, and on what terms.**

- **`collect`** — answers with a collection rather than a scalar, so it has no meaningful `having`, and binding a list needs a way to take one apart that §2.5 does not have. Adding it is a new `AggregateFunction` constant plus a value-semantics decision, not a reshape.
- **Sliding windows, and absence over time** — both need something to notice that time has passed *with no fact arriving*, which is exactly the input an engine that acts on fact movement never receives. Neither is possible without a clock or a caller-driven session time, and that is a §7.3 decision rather than an implementation detail: a clock read inside the engine makes the firing sequence depend on when it ran. §2.5's third amendment records what was chosen instead and what it costs.
- **Lookups into host-owned data during matching** — not built, and structurally so; §1's lookups bullet has the argument and the supported shape (a membership fact, `setField` plus `emit` for a rule that changes the list).
- **Backward chaining** — untouched. §1's forward-only decision stands.
- **Distributed evaluation** — §5's immutability split makes it *feasible* and that is all it makes it. A `CompiledRuleSet` is shippable to another JVM and §2.1's `(sessionId, handle)` is already the identity it would need, but the partitioning strategy, the wire protocol and cross-node fact routing are an architecture rather than a slice. Nothing here should be read as a start on it.

**Method note, because it changed what the phase found.** Every slice was mutation-tested: delete the feature, and see which tests fail. It repeatedly found tests that passed against a deleted feature — an equivalence assertion that could not fail, a refraction check attributed to the wrong cause, an assertion satisfied by boilerplate in the same message, a `before` operator no rule file exercised. It also killed two pieces of code no test could reach: a multi-support justification graph, and a cascade loop whose necessity `ACCUMULATE` then restored. A green suite says less than it appears to; a green suite against a deleted feature says nothing at all.
