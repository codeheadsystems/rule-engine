...
| 6 (stretch) | Negation/`NOT_EXISTS` ✔, `FOR_ALL` ✔, truth maintenance ✔, accumulate, temporal/CEP, distributed sharding | `NOT_EXISTS` is built (§1's amendment), so is `FOR_ALL` (§2.5's), and §4.4's amendment pays off the boundary both shipped with. The rest is out of scope; §1 says what each costs and what the interim answer is |
